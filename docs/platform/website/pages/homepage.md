---
status: Draft
version: "0.1"
updated: 2026-07-16
page: Acasă
route: /
evidence_map: ../content-evidence-map.md
---

# Copywriting — Homepage ezplay.org

## Rolul paginii

Homepage-ul poziționează EZPLAY ca organizație și platformă de educație antreprenorială prin experiență. El conduce către program, experiența introductivă și paginile pentru adulții care pot crea acces pentru tineri.

Homepage-ul nu începe cu jocul, autentificarea sau funcțiile platformei.

## SEO

### Titlu

> EZPLAY — Educație antreprenorială prin experiență

### Descriere

> EZPLAY dezvoltă programe de educație antreprenorială în care tinerii iau decizii, observă consecințele, învață din greșeli și încearcă din nou.

## Header

### Navigație

- Programul
- Cum învățăm
- Experiențe
- Pentru organizații
- Cercetare
- Despre EZPLAY

### Acțiune secundară

> Intră în platformă

### Selector limbă

- RO
- EN

## Secțiunea 1 — Hero

### Etichetă

> Program de educație antreprenorială pentru tineri

### H1

> **Descoperă antreprenoriatul. Prin experiență.**

### Introducere

> EZPLAY dezvoltă programe în care tinerii iau decizii, observă consecințele, învață din greșeli și încearcă din nou. Jocurile, simulările și provocările creează un mediu sigur în care ideile despre business devin experiențe pe care le poți analiza, înțelege și folosi.

### CTA principal

> Descoperă programul

### CTA secundar

> Adu EZPLAY în comunitatea ta

### Semnătură

> **WHERE FUTURE FOUNDERS START**

### Microcopy sub CTA

> Primul program EZPLAY este construit pentru tineri. Alte programe și publicuri vor fi dezvoltate în timp.

## Secțiunea 2 — Diferența EZPLAY

### Supratitlu

> Cunoașterea contează. Experiența o pune în mișcare.

### H2

> **Programul nu începe cu un răspuns. Începe cu o situație.**

### Text

> Ce faci când trebuie să alegi înainte să ai toate informațiile? Ce păstrezi, ce schimbi și la ce renunți? Cum reacționezi când o decizie care părea bună produce un rezultat neașteptat?

> În EZPLAY, participantul nu primește mai întâi toate concluziile. Intră într-o experiență, decide, vede ce se întâmplă și începe să pună întrebările potrivite. Teoria apare atunci când poate explica ceva trăit și poate ajuta la următoarea decizie.

### Mesaj evidențiat

> **Nu eliminăm cunoașterea. Îi dăm un context în care devine utilă.**

### CTA text

> Vezi cum învățăm

## Secțiunea 3 — Parcursul simplu

### H2

> **Decide. Observă. Înțelege. Încearcă din nou.**

### Introducere

> Experiențele EZPLAY sunt construite în jurul unui ritm simplu: participantul face o alegere, observă rezultatul, reflectează, primește instrumentele de care are nevoie și aplică din nou ceea ce a descoperit.

### Pasul 1

#### Titlu

> Intră în situație

#### Text

> Primești un context, resurse, obiective și limite. Nu există o singură cale evidentă.

### Pasul 2

#### Titlu

> Ia decizia

#### Text

> Alegi ce construiești, ce păstrezi, ce schimbi și ce risc îți asumi.

### Pasul 3

#### Titlu

> Observă consecințele

#### Text

> Rezultatul devine vizibil. Uneori confirmă strategia. Alteori arată ce nu ai luat în calcul.

### Pasul 4

#### Titlu

> Reflectează și înțelege

#### Text

> Compari deciziile, descoperi legături și folosești explicațiile care dau sens experienței.

### Pasul 5

#### Titlu

> Aplică din nou

#### Text

> Încerci o problemă nouă cu mai multă claritate, nu doar cu o definiție memorată.

## Secțiunea 4 — Primul program

### Etichetă

> În dezvoltare

### H2

> **Un program pentru tineri care vor să descopere cum se iau deciziile în business.**

### Text

> Primul program EZPLAY este construit în jurul unor experiențe educaționale numite Founder Rounds. Fiecare Round pornește de la o temă, o situație și o provocare. Participantul joacă, reflectează, primește informația relevantă și o pune la lucru într-o problemă nouă.

> Programul are un început comun, dar nu obligă toți participanții să urmeze același traseu pentru totdeauna. Pe măsură ce cresc experiența și autonomia, apar alegeri, conexiuni și provocări diferite.

### Pentru cine

> Nucleul programului este proiectat pentru elevi din clasele a V-a–a X-a. Experiențele pot fi adaptate în timp pentru participanți mai mici sau mai mari, în funcție de nivel, motivație și context.

### CTA principal

> Descoperă Programul EZPLAY pentru tineri

### CTA secundar

> Vezi experiențele

## Secțiunea 5 — Ce dezvoltă participantul

### H2

> **Nu urmărim răspunsul perfect. Urmărim o decizie mai bună.**

### Introducere

> Antreprenoriatul nu înseamnă să știi totul înainte să începi. Înseamnă să observi, să întrebi, să compari, să alegi și să îți asumi următorul pas.

### Card 1

#### Titlu

> Greșeala devine feedback

#### Text

> O decizie care nu funcționează nu închide experiența. Arată ce merită înțeles și schimbat la următoarea încercare.

### Card 2

#### Titlu

> Finance devine inteligibil

#### Text

> Profitul, Cash-ul și Cash Flow-ul nu sunt doar treaba contabilului. Ele spun povestea deciziilor unei companii.

### Card 3

#### Titlu

> Decizia nu așteaptă informația perfectă

#### Text

> Participantul învață să aleagă folosind ceea ce știe, să observe ce lipsește și să își adapteze strategia.

### Card 4

#### Titlu

> Compania este un sistem

#### Text

> Product, Market, Operations, Finance și Strategy nu funcționează separat. O alegere într-o zonă schimbă întregul rezultat.

### Card 5

#### Titlu

> Curajul se construiește prin acțiune

#### Text

> Încrederea nu vine din promisiunea că totul va merge bine, ci din experiența că poți observa, învăța și continua.

## Secțiunea 6 — Business Pillars

### H2

> **Cinci piloni. Aceeași companie.**

### Text

> Programul conectează cei cinci Business Pillars ai unei companii. Participantul poate avansa diferit în fiecare dintre ei și poate descoperi treptat cum se influențează reciproc.

### Strategy

> Direcția, alegerile, prioritățile și felul în care compania își construiește avantajul.

### Product

> Valoarea ofertei, diferențierea, calitatea și motivele pentru care cineva ar alege-o.

### Market

> Capacitatea companiei de a ajunge la oameni, de a câștiga clienți și de a construi relații cu piața.

### Operations

> Oamenii, procesele, resursele și sistemele prin care compania creează și livrează valoare.

### Finance

> Felul în care deciziile se transformă în Revenue, Expenses, Profit, Cash Flow, Cash și Equity.

### CTA

> Descoperă arhitectura programului

## Secțiunea 7 — Dincolo de joc

### H2

> **Jocul creează situația. Programul creează învățarea.**

### Text

> EZPLAY folosește jocuri, simulări și scenarii pentru a face deciziile vizibile. Dar experiența educațională nu se oprește atunci când se termină partida.

> Facilitatorul ajută participanții să analizeze ce s-a întâmplat. Informația relevantă explică problema. O nouă provocare cere aplicare. Progresul apare atunci când participantul poate folosi ceea ce a descoperit.

### Elemente

- simulare și decizie;
- Founder Debrief;
- Learning Input;
- Business Challenge;
- Level Up.

### CTA

> Vezi cum este construit un Founder Round

## Secțiunea 8 — Experiența introductivă

### Etichetă

> Primul pas

### H2

> **Joacă. Observă. Discută.**

### Text

> Înaintea unui Founder Round complet, participantul trebuie să cunoască instrumentul și modul de lucru EZPLAY. Experiența introductivă combină jocul cu un Founder Debrief: participanții învață regulile, iau primele decizii și discută ce au observat despre propriile strategii.

> Formatul este gândit pentru grupuri organizate în jurul unor mese de câte aproximativ patru participanți, cu un facilitator pentru una sau maximum două mese.

### CTA principal

> Descoperă experiența introductivă

### CTA secundar

> Solicită o discuție pentru organizația ta

## Secțiunea 9 — Pentru cine construim

### H2

> **O experiență bună pentru tineri are nevoie de adulți care îi creează loc.**

### Pentru tineri

#### Titlu

> Vreau să descopăr EZPLAY

#### Text

> Vezi ce fel de decizii vei lua, cum arată experiențele și de ce nu trebuie să știi deja totul despre business.

#### CTA

> Descoperă experiența ta

### Pentru părinți

#### Titlu

> Vreau să înțeleg valoarea programului

#### Text

> Descoperă ce urmărește programul, cum completează învățarea academică și ce nu promite EZPLAY.

#### CTA

> Informații pentru părinți

### Pentru școli și organizații

#### Titlu

> Vreau să creez acces pentru un grup

#### Text

> Vezi cum poate fi organizată o experiență, ce oferă EZPLAY și ce informații avem nevoie să aflăm despre participanți.

#### CTA

> EZPLAY pentru organizații

## Secțiunea 10 — Cercetare

### Supratitlu

> Experiența deschide întrebarea. Cercetarea ne ajută să construim răspunsul.

### H2

> **Construim cu curiozitate. Verificăm cu rigoare.**

### Text

> Programul folosește principii din curriculum spiral, educație bazată pe competențe, mastery learning, învățare bazată pe probleme și învățare prin experiență. Aceste repere ne ajută să proiectăm, dar nu validează automat ceea ce construim.

> De aceea separăm sursele externe, ipotezele EZPLAY și observațiile din experiențele reale. Publicăm ce ne influențează deciziile, ce limite are cercetarea și ce trebuie încă testat.

### CTA

> Explorează cercetarea EZPLAY

## Secțiunea 11 — Instrumentele

### H2

> **Instrumente care fac deciziile vizibile.**

### Text

> Deckbuilder-ul EZPLAY transformă o companie într-un sistem de resurse, clienți, costuri, investiții și alegeri. Tableau Builder explorează un nivel mai avansat al aceleiași fundații. Alte simulări și configurații pot susține în timp teme și experiențe diferite.

> Instrumentele sunt importante. Dar ele își ating scopul atunci când ajută participantul să observe, să întrebe și să decidă mai bine.

### CTA

> Descoperă instrumentele și simulările

## Secțiunea 12 — Dezvoltarea proiectului

### Etichetă

> EZPLAY este în dezvoltare

### H2

> **Nu construim în spatele unei uși închise.**

### Text

> Dezvoltăm programul, Founder Rounds, instrumentele și platforma împreună cu experiențele pe care le putem observa în realitate. Căutăm participanți curioși, părinți, educatori, facilitatori, antreprenori, cercetători și organizații care vor să contribuie cu timp, context, întrebări și feedback serios.

> A contribui nu înseamnă că orice idee devine automat parte din EZPLAY. Înseamnă că experiența și responsabilitatea pot avea un rol real în felul în care proiectul evoluează.

### CTA principal

> Construiește EZPLAY împreună cu noi

### CTA secundar

> Vezi ce dezvoltăm

## Secțiunea 13 — CTA final

### H2

> **Creează primul context în care un tânăr poate descoperi antreprenoriatul prin experiență.**

### Text

> Dacă reprezinți o școală, un club, un ONG sau o comunitate, spune-ne cu cine lucrezi și ce vrei să construiești. Vom începe de la participanți și de la experiența care le poate fi cu adevărat utilă.

### CTA principal

> Adu o experiență EZPLAY în comunitatea ta

### CTA secundar

> Contactează EZPLAY

## Footer — text de identitate

> EZPLAY dezvoltă programe de educație antreprenorială prin experiență, susținute de jocuri, simulări, facilitare și o platformă digitală.

## Footer — semnătură

> **WHERE FUTURE FOUNDERS START**

## Conținut care nu se publică încă

- număr de participanți;
- testimoniale;
- logo-uri de parteneri;
- prețuri;
- calendar;
- afirmații de validare;
- acces direct la programe online;
- sistem numeric de Founder Skills;
- Credits sau monede;
- certificări;
- informații juridice neverificate.
