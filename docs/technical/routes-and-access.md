...
- **Onboarding Obligatoriu**: Rutele protejate sunt condiționate de flag-ul `onboarding_completed` din tabela `user_profiles`. Dacă flag-ul este fals, middleware-ul interceptează navigarea (exceptând rutele externe/publice esențiale) și direcționează utilizatorul spre `/onboarding`.
