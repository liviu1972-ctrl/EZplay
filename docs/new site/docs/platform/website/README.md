...
Pachetul poate fi predat unui agent de implementare. Implementarea continuă pe platforma tehnică existentă; nu pornește ca proiect nou. Agentul trebuie să citească `existing-platform-continuity.md` și să inspecteze repository-ul tehnic separat înainte de instalări, reorganizări sau decizii privind versiunea framework-ului.
...
- `research/source-register.md` — ID-urile și starea linkurilor externe;
- `research/product/curriculum-progression-and-gamification.md` — analiza și limitele cercetării;
- `research/playtests/historical-session-evidence.md` — ce poate fi afirmat prudent despre experiențele istorice;
- `licensing/rights-and-provenance-register.md` — proveniența și drepturile activelor;
- `licensing/publication-permissions-register.md` — permisiunile pentru persoane, organizații, media și rezultate.
...
Se implementează fundația vizuală și homepage-ul ca prototip complet în repository-ul tehnic separat. După validarea limbajului vizual pe desktop și mobil, sistemul se extinde spre celelalte pagini în ordinea din `ux-ui/implementation-handoff.md`.
