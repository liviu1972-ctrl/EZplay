...
const DesktopGameViewClassic: React.FC<GameViewChildProps> = (props) => {
  const { 
  ...
  const { deck, discard, retiredCards, entrepreneur, cash, actionsRemainingThisTurn, lastAction, bonusBuy, retireFromHandBonus, cardChoices, copyCardState, activeConsultants } = activePlayer;
  ...
  return (
    <div className="w-full max-w-screen-2xl flex flex-col items-center space-y-2 animate-fade-in relative pt-4">
        <PlayerHUD players={players} activePlayerIndex={activePlayerIndex} isTurnTransitioning={isTurnTransitioning} onAdvanceToNextPlayer={onAdvanceToNextPlayer} />
        ...
        <div className="w-full flex justify-between items-start space-x-8">
            ...
            <div className="flex flex-col items-center space-y-2 flex-grow">
                <div className="w-full min-h-[183px] flex items-center justify-start space-x-4">
                    {entrepreneur && ( 
                        <div onClick={() => setIsManagementModalOpen(true)} className={`cursor-pointer transition-all duration-1000 ease-in-out relative group ${isSweeping ? 'translate-x-[950px] rotate-12' : 'translate-x-0 rotate-0'}`}>
...
export default DesktopGameViewClassic;
