...
export function CurriculumHeader({ dict }: { dict?: any }) {
  ...
  const NavLinks = ({ mobile = false }: { mobile?: boolean }) => {
    const linkClass = mobile
      ? "block px-2 py-2 text-lg font-medium text-[#B2C0B8] hover:text-white hover:text-brand-orange transition-colors"
      : "flex items-center text-sm font-medium text-[#B2C0B8] hover:text-white transition-colors"
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 w-full border-b border-[#1A363A] bg-[#0D2427]/95 text-white backdrop-blur supports-[backdrop-filter]:bg-[#0D2427]/60 transition-transform duration-300 ${isVisible ? "translate-y-0" : "-translate-y-full"}`}>
        ...
        <div className="w-full max-w-[1600px] mx-auto flex items-center justify-between px-4">
          <div className="flex gap-6 lg:gap-10 items-center">
            ...
            <div className="hidden md:flex">
              <Button variant="ghost" className="text-sm font-medium text-[#B2C0B8] hover:text-white hover:bg-white/10" render={<Link href="/program" />}>
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet>
              <SheetTrigger render={
                <Button variant="outline" className="flex items-center gap-2 rounded-full px-4 border-[#1A363A] text-white hover:bg-white/10">
                  <Menu className="h-4 w-4" />
                  <span className="hidden sm:inline">Meniu</span>
                </Button>
              } />
              <SheetContent side="right" className="flex flex-col gap-4 bg-[#0D2427] text-white border-l border-[#1A363A] z-[100]">
                <SheetHeader>
